...
#[repr(C)]
#[allow(missing_docs)]
#[derive(Default, zerocopy::Immutable, zerocopy::FromBytes, zerocopy::IntoBytes)]
...
impl Header {
    ...
    pub fn new_ustar() -> Header {
        let mut header = Header { bytes: [0; 512] };
        {
            let h: &mut UstarHeader = zerocopy::transmute_mut!(&mut header);
            h.magic = *b"ustar\0";
            h.version = *b"00";
        }
        header.set_mtime(0);
    ...
    pub fn from_byte_slice(bytes: &[u8]) -> &Header {
        &Header::ref_from_bytes(bytes).unwrap()
    }
